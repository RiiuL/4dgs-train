...
def parse_args():
    ...
    parser.add_argument("--out_dir", type=str, default="./output_4dgs")
...
def train(args, cameras, init_points, init_scales=None):
    ...
    print(f"\n{'=' * 60}")
    print(f"  4DGS Array Camera Training (v4)")
    ...
    print(f"  Warm-up: {args.warmup_iters} iters (static)")
    ...
    optimizer = torch.optim.Adam([
        ...
        {'params': [model.scales], 'lr': args.lr_scale, 'name': 'scales'},
    ...
    # LR scheduler: exponential decay to 1% of initial LR by end
    lr_decay_rate = (0.01) ** (1.0 / args.iters)  # per-step multiplier
    scheduler = torch.optim.lr_scheduler.ExponentialLR(optimizer, gamma=lr_decay_rate)

    # Scale clamp: adaptive based on init scale
    # Gaussians should not grow more than ~4x their initial size
    if init_scales is not None:
        init_log_scale_mean = np.log(np.clip(init_scales, 0.01, 50.0)).mean()
        SCALE_MAX = init_log_scale_mean + 1.5  # ~4.5x initial size
    else:
        SCALE_MAX = 1.0  # exp(1)=2.7mm max for random init
    SCALE_MIN = -6.0  # exp(-6)=0.0025mm
    print(f"  Scale clamp: exp({SCALE_MIN:.1f})={math.exp(SCALE_MIN):.4f}mm ~ "
          f"exp({SCALE_MAX:.1f})={math.exp(SCALE_MAX):.2f}mm")
    ...
    for step in pbar:
        ...
        # Warm-up: deformation OFF
        use_deform = (step > args.warmup_iters)

        # Freeze/unfreeze deformation net
        if step == 1:
            for p in model.deform_net.parameters():
                p.requires_grad = False
        if step == args.warmup_iters + 1:
            print(f"\n  >>> Warm-up complete. Enabling deformation network.")
            for p in model.deform_net.parameters():
        ...
        for ci in batch_idx:
            ...
            if use_deform:
                ...
                for t in time_steps:
                    ...
                    blur_sim = blur_sim + r / args.integration_steps
            else:
                t_mid = (t_start + t_end) / 2.0
                pos_t, col, opa, sca, rot = model.get_gaussians_at(t_mid, use_deform=False)
                blur_sim, _, _ = render(pos_t, col, opa, sca, rot,
                                        args.img_h, args.img_w, fx, fy,
                                        cam_tx, cam_ty, cam_tz,
                                        render_depth=False, z_max=args.z_max)
        ...
        if use_deform and args.lambda_deform_reg > 0:
        ...
        # Scale regularization: penalize large scales ¡æ force blur from motion, not size
        if args.lambda_scale_reg > 0:
            # exp(scales) = actual size in mm. Penalize large actual sizes.
            scale_penalty = torch.exp(model.scales).mean()
            total_loss = total_loss + args.lambda_scale_reg * scale_penalty

        # Opacity regularization: push toward 0 or 1 (binary)
        # entropy: -p*log(p) - (1-p)*log(1-p), minimize = encourage binary
        ...
        # === Scale clamp: prevent Gaussians from growing/shrinking unboundedly ===
        with torch.no_grad():
        ...
        if step % args.save_every == 0:
            _save_debug(model, cameras[0], fx, fy, args, debug_dir, step, use_deform)

    # Restore best model if training diverged at the end
    if avg_loss > best_loss * 1.5 and best_loss < float('inf'):
